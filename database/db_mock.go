...
func getTestMessagesModels() []*messageModel {
	var gms []*messageModel
	var gm *messageModel
	gm, _ = newMessageModel(&models.Message{
...
func initTestMessageService() *MessageService {
	os.Setenv("ENV", "test")
...
func setupTestMessages() *MessageService {
	os.Setenv("ENV", "test")
	os.Setenv("MONGO_URI", "mongodb+srv://in_mem")
	os.Setenv("DATABASE", "test")
	db, _ := initializeNewTestClient()
	...
	td := getTestMessagesModels()
	for _, d := range td {
		_, err := ts.MessageCreate(d.toRoot())
		if err != nil {
			panic(err)
		}
	}
...
func newTestMongoDatabase(databaseName string) (*testMongoDatabase, error) {
	...
	testMessageCollection, err := newTestMongoCollection("messages")
	if err != nil {
...
func (db *testDBClient) NewMessageHandler() *DBHandler[*messageModel] {
	col := db.GetCollection("messages")
	return &DBHandler[*messageModel]{
		db:         db,
		collection: col,
	}
}
